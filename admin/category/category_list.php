...
<style>          
     ...
     <div class="table-responsive">	
        <table align="right">
              <TR>
                <TD  nowrap="nowrap">
                  <?php
                      $hash    =  getTableFieldsName("category");
 ...
 <a href="index.php?cmd=edit"  class="btn btn-primary"><i class="fas fa-plus"></i>Add a Category</a> <br><br>
                    ...
                    <tbody>
					 ...
					 <?php
							...
							if($_SESSION["search"]=="yes")
							  ...
							  $whrstr .= " AND parent_category_id='0'";
							...
							for($j=0;$j<count($arr2);$j++)
							{
                               ...
                               $tree = get_tree($arr2[$j]['id'],$db);
							   $node_list='';
							   foreach($tree as $key=>$value)
							   {
							     $node_list .= "'".$key."',";
							   }	  
							   $node_list = substr($node_list,0,strlen($node_list)-1);	
							   
								unset($info);		  
							 $info["table"] = "category";
						     $info["fields"] = array("category.*"); 
							 $info["where"]   = "1   AND cat_name in($node_list)  ORDER BY id DESC";
							 $arr =  $db->select($info);							
					 ...
					 ?>
						<tr bgcolor="<?=$row?>" onmouseover=" this.style.background='#ECF5B6'; " onmouseout=" this.style.background='<?=$row?>'; ">
						  <td>
                              <?php
							  if(is_file('../../'.$arr[$i]['file_icon'])&&file_exists('../../'.$arr[$i]['file_icon']))
							  {
							 ?>
                              <img src="../../<?=$arr[$i]['file_icon']?>"  style="width:50px;height:50px;">
                              <?php
							  }else
							  {
							?>  
                             <img src="../../images/no_image.jpg" style="width:50px;height:50px;">
                            <?php
							  }
							?>  
                          </td>
                          <td>
				               <?php
										...
										$info2['where']    =  "1 AND id='".$arr[$i]['parent_category_id']."' LIMIT 0,1";
										$res2  =  $db->select($info2);
										echo $res2[0]['cat_name'];	
				                ?>
						  </td>
						  <td>
						       <a href="javascript:void();" onclick="window.open('index.php?cmd=view_tree&id=<?=$arr[$i]["id"]?>','Tree','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no, width=520,height=500,left=430,top=23'); return false;" class="btn blue">
						         <?=$arr[$i]['cat_name']?>
						       </a>	
						  </td>			  
						  <td nowrap >
